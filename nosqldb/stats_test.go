...
func TestPreparedStatementDoesWritesUsesOperation(t *testing.T) {
	...
	assert.False(t, (*PreparedStatement)(nil).doesWrites())
}

func TestQueryStatsMetadataUsesPreparedOperation(t *testing.T) {
	statement := "DECLARE $val INTEGER; UPDATE stats_table SET value = $val WHERE id = 1"
	req := &QueryRequest{
		Statement: statement,
	}

	assert.False(t, req.queryStatsMetadata().doesWrites)

	req.PreparedStatement = &PreparedStatement{
		sqlText:   statement,
		operation: queryOperationUpdate,
	}
	assert.True(t, req.queryStatsMetadata().doesWrites)
}
